...
fn build_prompt(command: &str, language: &str) -> String {
    format!(
        "你是 Linux 命令笔记助手。请为命令 `{command}` 生成一份可读性高的 Markdown 速查笔记；输出语言为 {language}；必须包含简介、常用参数、至少5个带说明的示例、注意事项；常用参数可用无序列表或表格；示例命令必须使用带语言标记且闭合的代码块（例如以 ```bash 开头并以 ``` 结束）；输出纯 Markdown，不要附加解释。"
    )
}
