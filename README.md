...
- 删除：`gg rm <cmd>...`（不可逆，会先确认；非交互终端需显式 `--yes`）
...
```bash
gg rm ls                 # 询问后删除
gg rm ls grep awk        # 一次删多个
gg -y rm ls              # 跳过询问（脚本里用）
```

删除不可逆，所以规则比查询严：

- **默认否定** —— 询问符是 `[y/N]`，直接回车等于不删
- **非交互终端拒绝执行** —— 管道或重定向下没有用户可确认，直接跳过并返回
  退出码 3，除非显式加 `-y/--yes`。与 AI 回退同一条原则：没拿到明确同意
  就不做有副作用的操作
- **不做部分删除** —— 先校验并收集全部目标，避免「删了几个才发现有笔误」
- 目标不存在时报告未命中并返回退出码 3，不会静默成功
- 不会删除同名**目录**，命令名中的路径分隔符一律拒绝
- 若笔记目录位于 git 仓库内，删除后会提示可用的找回命令
...
| 2 | 命令行用法错误（clap 输出） |
| 3 | 没有产生任何结果：查询未命中，或操作因缺少用户确认而未执行 |
...
- 被保留字占用的命令名（典型是 `rm`）仍可用 `gg show <cmd>` 查询，
  `--browser` / `--edit` 照样生效
