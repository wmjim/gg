...
`-b` 与 `-e` 只修饰**查询动作**（`gg <命令>` 与 `gg show <命令>`），两者互斥：
...
`--set-editor` 的取值定位不到可执行文件时会当场提示，但**仍会保存**：运行时
...
提示词内置在程序里；在配置目录放一份 `AGENTS.md` 即可整体替换（文件不存在或内容只有空白时用内置默认）：
...
1 与 2 的分界不是「谁报的错」，而是**错误来自哪里**：命令行本身用错（含
...
- `gg search` 的关键词不能为空（含只由空白组成），否则按用法错误退出（2）
- 帮助信息与全部运行时提示均已本地化；但**命令行用法错误**（缺参数、未知选项等）
  由 `clap` 生成，目前仍为英文
